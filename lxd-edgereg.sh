...
cat <<EOF |sed -r 's/[ \t]+$//g;/## /d'|lxc profile edit ${NAME}
config:
  boot.autostart: "true"
  ...
  user.user-data: |
    ...
    users:
      - name: ubuntu
        ...
        lock_passwd: false
        ## Salted Password: `echo ubuntu|mkpasswd --method=SHA-512 --rounds=4096 -`
        ## Password is ubuntu
        passwd: \$6\$rounds=4096\$gdFl/jr4d8H1YiPr\$gmWxVI1VLT1Woe3CgJxtaTTknKVrEejshZNbDx89/ic1C2FhJL7YW1/gWn1cWFHVqWY91J7KQOoeBjy8JmLvn0
      ...
      - name: ${ME}
        ...
        lock_passwd: false
        ## Salted Password: `echo demo123|mkpasswd --method=SHA-512 --rounds=4096 -`
        ## Password is demo123
        passwd: \$6\$rounds=4096\$JvqY7nGYJJLoAPgw\$ibqCK752ye/.XXsRQsAio4cyCu9eKrL/5N7aapeATPmHNKW9Y/zk4/f5DWfRmd7ygHVf/HjjToSzqeWPzWoEo.
        sudo: ALL=(ALL) NOPASSWD:ALL
        shell: /bin/bash
        ssh_import_id:
          - lp:${LPID}
